...
public class LoremController extends PlatformController {
	...
	public void generateQuestions() {
		...
		try {
			...
			response = Unirest
					.get("https://montanaflynn-lorem-text-generator.p.mashape.com/paragraph?count=" + (howMany * 28) + "&length=3")
					.header("X-Mashape-Key", "sDUMNakHLamsh2xKOnuIQWteLdN4p1n4uE6jsnFjX0tUBfIPx9").header("Accept", "application/json")
					.asJson();
			...
			for (int i = 0; i < howMany; i++) {
				...
				if (q.getBoolean("is_published")) {
					q.setLong("popular_votes", (random.nextInt(3) * (random.nextInt(23) * random.nextInt(17)) + random.nextInt(19))
							+ random.nextInt(13));
